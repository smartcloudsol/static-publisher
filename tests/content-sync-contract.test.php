...
content_sync_expect(str_contains($journal, "add_action('smartcloud_static_publisher_resource_changed_v1'"), 'Provider-owned resources must enter the durable journal.');
content_sync_expect(str_contains($journal, "'resourceRenderUrls'"), 'Resource journal events must carry explicit render URLs.');
content_sync_expect(str_contains($journal, "'resourceDeleteUrls'"), 'Resource journal events must carry exact delete URLs.');
content_sync_expect(str_contains($journal, '$after = $deleteUrls && !$renderUrls ? null : $base;'), 'Resource deletion events must not restore a deleted source projection while journal events are folded.');
content_sync_expect(str_contains($journal, "operation = 'resource'"), 'Resource-only journal events must be visible independently of post-type scope.');
content_sync_expect(str_contains($plugin, 'smartcloud_static_publisher_resource_change_token_data'), 'Generic resource providers need a stable change-token hook.');
...
content_sync_expect(str_contains($journal, "'/privacy/cookie-observations'"), 'The authenticated cookie-observation bridge must be registered.');
content_sync_expect(str_contains($journal, "'smartcloud_static_publisher_cookie_observations_v1'"), 'Cookie observations must be handed to an explicit privacy provider contract.');
