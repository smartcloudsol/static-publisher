...
const requireExporter = createRequire(
  new URL("../../exporter/package.json", import.meta.url),
);
const { build } = requireExporter("esbuild");
const { chromium } = requireExporter("playwright");
const ts = requireExporter("typescript");
const contractSource = readFileSync(
  new URL("../../exporter/src/content-sync-contract.ts", import.meta.url),
  "utf8",
);
...
const {
  canonicalizeContentSyncScope,
  contentSyncScopeFingerprint,
  contentSyncTargetFingerprint,
  sha256Fingerprint,
} = await import(
  `data:text/javascript;base64,${Buffer.from(contractCode).toString("base64")}`
);
...
const mocks = {
  ...
  "@smart-cloud/wpsuite-core":
    "export const getWpSuite = () => window.WpSuite;",
...
const bundle = await build({
  ...
  stdin: {
    contents:
      "import '@mantine/core/styles.css'; import '@mantine/notifications/styles.css'; import './src/index.tsx';",
    resolveDir: adminDir,
    loader: "tsx",
  },
  bundle: true,
  write: false,
  outdir: "/tmp/static-publisher-admin-ui-test",
  jsx: "automatic",
  define: { __WPSUITE_PREMIUM__: "true", "process.env.NODE_ENV": '"test"' },
  plugins: [
    {
...
const longConsumerId =
  "content-sync:64a976a57b30a414fbbd314318b474899adb974efdd5fa3f8af1c58832d67e40";
const distinctRetryError =
  "The content-sync request failed after retrying https://source.example.test/wp-json/smartcloud-static-publisher/v1/content-sync/acknowledgement because the connection timed out.";
...
function runtimeFixture(config, currentStatus, cardPresentation) {
  ...
  for (const [rule, status, consumerId] of [
    [
      { ...savedRule, postTypes: ["post"] },
      currentStatus === "ready" ? "required" : "ready",
      "historical-scope",
    ],
    [savedRule, currentStatus, "current-saved-scope"],
  ]) {
    const scopeFingerprint = contentSyncScopeFingerprint(
      config.sourceOrigin,
      rule.id,
    ...
    rules[coalesceKey] = {
      ...
      baselineReason:
        status === "required" ? `${consumerId} needs a baseline` : null,
    };
    if (status === "ready")
      entries[coalesceKey] = {
        ruleId: rule.id,
  ...
  await page.addInitScript(
    ({ holdAccess, lambdaConfigOverride }) => {
      ...
      window.fixtureConfig = {
        ...
        remoteWorkers: {
          render: { enabled: false, concurrency: 4, maxAttempts: 2 },
        },
        maxPages: 0,
        verbose: false,
        logLevel: "info",
        s3SyncMode: "sdk-upload-only",
        s3: {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (
      ...
      json = {
        ...
        queueItems: [],
        queueLength: 0,
        currentRun: currentProgress
          ? {
              id: "fixture-publish",
              command: "publish",
              crawlMode: "incremental",
              status: "running",
              createdAt: "2026-09-19T13:57:42+00:00",
            }
          : null,
        currentProgress,
        currentCrawlEvent: null,
        lastRun: null,
        schedulerState: {},
        lockActive: false,
        contentSync: currentBaselineStatus
          ? runtimeFixture(config, currentBaselineStatus, cardPresentation)
          : null,
      };
    else if (path.includes("post-types"))
      json = {
        items: [
          {
            slug: "page",
      ...
      json = {
        ...
        items: ["idle", "retry-wait", "baseline-required"].map((status, i) => ({
          id: String(i),
          occurredAt: "2026-09-06T07:27:11.807Z",
          eventType: "content-sync-baseline-required",
          status,
          jobId: "fixture-publish-job",
          command: "publish",
          actor: "queue-runner",
          message:
            "No verified normal-release baseline exists for this rule scope.",
...
try {
    ...
    await test(`${name}: audit scrolls with complete statuses; scheduler actions stay reachable`, async () => {
      ...
      try {
        ...
        for (const status of ["idle", "retry-wait", "baseline-required"]) {
          const badge = audit.locator(".mantine-Badge-label", {
            hasText: status,
          });
        ...
        const scheduler = page.getByRole("region", {
        ...
        await edit.click();
        await page
          .getByRole("dialog", { name: "Edit scheduler rule" })
          .waitFor();
        assert.equal(
          await page.getByLabel("Rule ID", { exact: true }).inputValue(),
          "hourly-content-sync",
        );
  ...
  await test("Lambda deploy progress counts completed results instead of staged files", async () => {
    ...
    try {
      ...
      assert.match(
        bodyText,
        /Target deployment: 700\/1038 \(67\.4%\)/u,
        bodyText,
      );
      assert.match(bodyText, /Remote batches completed: 14\/21/u, bodyText);
      assert.match(
        bodyText,
        /Received 700\/1038 Lambda result\(s\): copied 0, skipped 700, failed 0\./u,
        bodyText,
      );
      assert.deepEqual(errors, []);
    } finally {
  ...
  await test("delayed paid access updates an untouched crawl default to incremental", async () => {
    ...
    try {
      assert.equal(
        await page.getByRole("textbox", { name: /^Crawl mode/ }).inputValue(),
        "full",
      );
      await page.waitForFunction(
        () => typeof window.releaseAccess === "function",
      );
      await page.evaluate(() => {
        window.holdAccess = false;
        window.releaseAccess();
      });
      await page.waitForFunction(() =>
        document.querySelector('input[value="incremental"]'),
      );
      assert.equal(
        await page.getByRole("textbox", { name: /^Crawl mode/ }).inputValue(),
        "incremental",
  ...
  await test("explicit full survives delayed access, PRO save refresh, and command changes", async () => {
    ...
    try {
      ...
      await page
        .getByRole("button", {
          name: "Save PRO Scheduler Settings",
          exact: true,
        })
        .click();
  ...
  for (const status of ["ready", "required"]) {
    await test(`baseline status uses saved scope (${status}), ignoring historical and unsaved scopes`, async () => {
      ...
      try {
        ...
        assert.equal(
          await page.getByText("historical-scope", { exact: true }).count(),
          0,
        );
        assert.equal(
          await page
            .getByText(
              status === "ready" ? "baseline ready" : "baseline stale",
              { exact: true },
            )
            .count(),
          1,
        ...
        assert.equal(
          await page
            .getByText("New baseline required", { exact: true })
            .count(),
          status === "required" ? 1 : 0,
        );
        await page.getByText("Scheduler Settings", { exact: true }).click();
        ...
        await page
          .getByRole("option", { name: "Page (page)", exact: true })
          .click();
        await modal
          .getByRole("textbox", { name: "Public post types" })
          .press("Escape");
        await modal
        ...
        assert.match(
          await scheduler.locator("tbody tr td").nth(5).innerText(),
          /^post;/,
        );
    ...
    for (const errorKind of ["duplicate", "distinct"]) {
      await test(`${width}px baseline card wraps complete status and ${errorKind} errors without clipping`, async () => {
        ...
        try {
          ...
          assert.equal(
            await card.getByText(longBaselineReason, { exact: true }).count(),
            1,
          );
          assert.equal(
            await card.getByText(distinctRetryError, { exact: true }).count(),
            errorKind === "distinct" ? 1 : 0,
          );
          await card
            .getByText("Baseline verified: 2026-09-06T15:00:00.123Z", {
              exact: true,
            })
            .waitFor();
          await card.getByText("Retry attempt: 12", { exact: true }).waitFor();
          await card
            .getByText("Next retry: 2026-09-06T17:45:12.123Z", { exact: true })
            .waitFor();
          const badge = card
            .locator(".mantine-Badge-label")
            .filter({ hasText: /^baseline stale$/ });
          assert.equal(await badge.textContent(), "baseline stale");
          assert.ok(
            await badge.evaluate(
              (element) => element.scrollWidth <= element.clientWidth + 1,
            ),
            "Complete baseline stale label must fit its badge",
          );
          await card.screenshot({
            path: `/tmp/static-publisher-baseline-${width}-${errorKind}.png`,
          });
          const geometry = await card.evaluate((element) => {
            ...
            for (const child of element.querySelectorAll("*")) {
              ...
              if (
                rect.width &&
                (rect.left < left - 1 || rect.right > right + 1)
              ) {
                outside.push({
                  type: child.tagName,
                  text: child.textContent.slice(0, 90),
                  left: rect.left,
                  right: rect.right,
                });
              }
          ...
          assert.deepEqual(
            geometry.outside,
            [],
            `All card descendants and text must fit the content rail (${width}px)`,
          );
          assert.ok(
            geometry.scrollWidth <= geometry.width + 1,
            "The baseline card must not overflow horizontally",
          );
          assert.ok(
            await page.evaluate(
              () =>
